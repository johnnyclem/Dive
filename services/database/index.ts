...
interface ApiMessageResponse {
  ...
  content: string;
  createtime: string;
  role: 'user' | 'assistant'; // Assuming roles are limited
  mcp_session_id: string;
  // Add other expected fields if known
}
...
class DirectDatabaseAccess implements DatabaseOperations {
  ...
  constructor(dbPath?: string) {
    try {
      const finalDbPath = dbPath || "data/database.sqlite";
      ...
      // Database initialized. Migrations should be run explicitly by the application entry point.
      // sqlite.exec(`
  ...
  async saveChat(chatId: string, _options?: DatabaseOptions) {
    const chat = await this.db.query.chats.findFirst({
      where: eq(chats.id, chatId),
    });
...
class ApiDatabaseAccess implements DatabaseOperations {
  public db: BetterSQLite3Database<typeof schema> | null;
  ...
  async getAllChats(options?: DatabaseOptions) {
    ...
    const chats = resBody.data.map((chat: ApiChatResponse) => ({
  ...
  async getChatWithMessages(chatId: string, options?: DatabaseOptions) {
    ...
    const messages = messageResBody.data.map((message: ApiMessageResponse) => ({
