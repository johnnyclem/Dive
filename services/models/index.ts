...
export class ModelManager {
  ...
  async initializeModel(): Promise<BaseChatModel | null> {
    ...
    let modelSettings = (config as iModelConfig).configs[activeProvider];
    this.enableTools = (config as iModelConfig).enableTools ?? true;

    // Fallback to Venice if no provider selected or settings missing
    if (!modelSettings) {
      const veniceEndpoint = process.env.VENICE_ENDPOINT;
      const veniceApiKey = process.env.VENICE_API_KEY;
      if (veniceEndpoint && veniceApiKey) {
        logger.warn(`Model settings not found for provider: ${activeProvider}, falling back to Venice`);
        modelSettings = {
          model: process.env.VENICE_MODEL || 'gpt-3.5-turbo',
          modelProvider: 'openai',
          apiKey: veniceApiKey,
          configuration: { baseURL: veniceEndpoint },
        };
