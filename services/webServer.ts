...
export class WebServer {
  ...
  private setupRoutes() {
    ...
    // Add a web search API endpoint that handles knowledge base results or web search results
    this.app.post("/api/web_search", async (req, res) => {
      try {
        const { query, fallback_to_web, knowledge_base_results } = req.body;
        
        // If knowledge base results were provided, return them directly
        if (knowledge_base_results) {
          logger.info(`Returning knowledge base results for query: ${query}`);
          return res.json({ 
            success: true, 
        ...
        try {
          ...
          // For demo, we'll just create mock results
          const searchResponse = {
            results: [
              {
                title: `Search result for "${query}"`,
                url: `https://example.com/search?q=${encodeURIComponent(query)}`,
                snippet: `This is a search result for "${query}". In a real implementation, this would come from a search API.`
              },
              {
                title: `Another result for "${query}"`,
                url: `https://example.org/results?q=${encodeURIComponent(query)}`,
                snippet: `More information about "${query}" would be shown here from a search engine.`
              }
            ]
          };
    ...
    // Add a dedicated knowledge base search API endpoint
    this.app.post("/api/knowledge_search", async (req, res) => {
      try {
        const { query, knowledge_base_id } = req.body;
        
        logger.info(`Knowledge base search for query: ${query}`);
        ...
        // Import directly to prevent window.electron issues in Node.js context
        const { getActiveKnowledgeBase, searchKnowledgeBase } = await import('../electron/main/knowledge-store.js');
        
        // Use the active knowledge base if none specified
        const kbId = knowledge_base_id || getActiveKnowledgeBase();
        
        if (!kbId) {
          return res.status(400).json({
            success: false,
            message: "No knowledge base ID provided and no active knowledge base"
          });
        }
        
        // Perform the search
        try {
          const searchResults = searchKnowledgeBase(query, kbId, 5);
          ...
          logger.error(`Knowledge base search error: ${searchError}`);
          return res.json({
            success: false,
        ...
        logger.error(`Knowledge search error: ${error instanceof Error ? error.message : String(error)}`);
        return res.status(500).json({
          success: false,
          message: error instanceof Error ? error.message : "Unknown error occurred"
        });
    ...
    this.app.post("/api/chat", (req, res) => {
      this.upload(req, res, async (err) => {
        ...
        try {
          const { message, chatId, fingerprint, user_access_token, personaPrompt } = req.body;
          ...
          await this.mcpClient.processQuery(chatId, queryInput, onStream, undefined, fingerprint, user_access_token, personaPrompt);
    ...
    this.app.post("/api/chat/edit", (req, res) => {
      this.upload(req, res, async (err) => {
        ...
        try {
          const { chatId, messageId, content, fingerprint, user_access_token, personaPrompt } = req.body;
          if (getDatabaseMode() === DatabaseMode.API) {
            if (!fingerprint && !user_access_token) {
              return res.status(400).json({
                success: false,
    ...
    this.app.post("/api/chat/retry", async (req, res) => {
      const { chatId, messageId, fingerprint, user_access_token, personaPrompt } = req.body;
      if (getDatabaseMode() === DatabaseMode.API) {
        if (!fingerprint && !user_access_token) {
          return res.status(400).json({
            success: false,
      ...
      try {
        const onStream = this.setupSSE(res);
        await this.mcpClient.processQuery(chatId, "", onStream, messageId, fingerprint, user_access_token, personaPrompt);
        res.write("data: [DONE]\n\n");
        res.end();
      } catch (error) {
        this.handleStreamError(error as Error, res);
