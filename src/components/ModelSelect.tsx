...
const ModelSelect = () => {
  ...
  const handleModelChange = async (value: string) => {
    ...
    const verifiedList = allVerifiedList[value] ?? {}
    const activeConfig = configList?.[value]
    const supportTools = verifiedList[activeConfig?.model as string]?.supportTools ?? false

    try {
      const data = await saveAllConfig({ providerConfigs: configList as InterfaceModelConfigMap, activeProvider: value as InterfaceProvider, enableTools: supportTools })
      if (data.success) {
        showToast({
