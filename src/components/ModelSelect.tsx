...
const ModelSelect = () => {
  ...
  const handleModelChange = async (key: React.Key) => {
  ...
  const selectedModelDetails = modelList.find(m => m.key === model);
    ...
    <div className="flex items-center space-x-2">
      <Dropdown>
        <DropdownTrigger>
          <Button
            variant="bordered"
            className={`${modelList.length === 0 ? "disabled" : ""} justify-start`}
            disabled={modelList.length === 0}
            style={{ maxWidth: '250px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
          >
            {selectedModelDetails ? (
              <div className="flex items-center" key={selectedModelDetails.key}>
                <img
                  src={PROVIDER_ICONS[selectedModelDetails.provider.replace("-", "_") as keyof typeof PROVIDER_ICONS]}
                  alt={selectedModelDetails.provider}
                  className={`w-5 h-5 mr-2 ${isProviderIconNoFilter(selectedModelDetails.provider) ? "filter-none" : "invert dark:invert-0"}`}
                />
                <span className="truncate">
                  {optionMask(selectedModelDetails.name)}
                </span>
              </div>
            ) : (
              modelList.length === 0 ? t("models.noModelAlertOption") : t("models.selectModelPlaceHolder")
            )}
          </Button>
        </DropdownTrigger>
        <DropdownMenu
          ...
          disabledKeys={modelList.length === 0 ? ["__placeholder__"] : []}
        ...
        >
          {(item) => (
            <DropdownItem
              ...
              startContent={
                <img
                  src={PROVIDER_ICONS[item.provider.replace("-", "_") as keyof typeof PROVIDER_ICONS]}
                  alt={item.provider}
                  className={`w-5 h-5 ${isProviderIconNoFilter(item.provider) ? "filter-none" : "invert dark:invert-0"}`}
                  style={{ marginRight: '8px' }}
                />
              }
            >
              {optionMask(item.name)}
            </DropdownItem>
          )}
        </DropdownMenu>
      </Dropdown>
      ...
      >
        ...
        >
          <svg width="20px" height="20px" viewBox="0 0 20 20" className="fill-current">
            <g id="surface1">
