...
export const loadConfigAtom = atom(
  ...
  async (get, set) => {
    try {
      ...
      console.warn("fetching models from /api/config/model")
      ...
      const data = await response.json() as ApiResponse<RawModelConfig>
      ...
      console.warn("Failed to load config:", apiError)
...
export async function verifyModelWithConfig(config: InterfaceModelConfig, signal?: AbortSignal) {
  ...
  return await apiFetch("/api/modelVerify", {
    signal,
    method: "POST",
    headers: {
      "Content-Type": "application/json",
