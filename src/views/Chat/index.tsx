...
const ChatWindow = () => {
  ...
  const addChatToHistory = useSetAtom(addChatToHistoryAtom)
  ...
  useEffect(() => {
    if (!chatId) {
      ...
      // Clear the current chat ID to force new canvas creation
      setCurrentChatId('');
    }
  }, [chatId, setCurrentChatId]);

  // Make sure we're setting the chat ID atom for new chats
  useEffect(() => {
    // When navigating away from a chat (to root or new chat route)
    if (!chatId && currentChatId.current) {
      // Save the current chat before clearing
      const saveCurrentChat = async () => {
        try {
          const response = await fetch(`/api/chat/${currentChatId.current}/save`, {
            method: 'POST',
          });
          if (!response.ok) {
            console.warn('Failed to save chat before navigation');
          }
        } catch (error) {
          console.warn('Error saving chat before navigation:', error);
        }
      };
      saveCurrentChat();
      ...
      if (currentChatId.current) {
        const saveCurrentChat = async () => {
          try {
            ...
            if (!response.ok) {
              console.warn('Failed to save chat before unmount');
            }
          } catch (error) {
            console.warn('Error saving chat before unmount:', error);
          }
        };
        saveCurrentChat();
      }
  ...
  const handlePost = useCallback(async (body: any, type: "json" | "formData", url: string) => {
    try {
      // Use the already imported interceptor
      const response = await injectPersonaToRequest(url, {
        method: "POST",
        headers: type === "json" ? {
          ...
          try {
            ...
            switch (data.type) {
              ...
              case "chat_info":
                ...
                setCurrentChatId(data.content.id)
  ...
  }, [addChatToHistory, navigate, scrollToBottom, setCurrentChatId, setIsChatStreaming, showToast, t])
  ...
  }, [isChatStreaming, scrollToBottom, setCurrentChatId, addChatToHistory, handlePost])
