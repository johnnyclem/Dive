...
import configs from "../../../../electron/main/default-models.json"
import { writeRawConfigAtom } from "../../../atoms/configState"
import type { InterfaceModelConfigMap } from "../../../atoms/configState"

// Define type for the bundled default-models.json structure
interface DefaultModels {
  configs: InterfaceModelConfigMap
  activeProvider: InterfaceProvider
  enable_tools?: boolean
  enableTools?: boolean
}

// Cast raw JSON to typed default models
const defaultModels = configs as unknown as DefaultModels

const PageLayout = () => {
  const { t } = useTranslation()
  const restoreDefaults = useSetAtom(writeRawConfigAtom)
  ...
  // Handler to restore default models
  const handleRestoreDefaults = async () => {
    try {
      await restoreDefaults({ providerConfigs: defaultModels.configs as unknown as InterfaceModelConfigMap, activeProvider: defaultModels.activeProvider })
      showToast({ message: t("models.restoreSuccess"), type: "success" })
    } catch (error) {
      console.error("Failed to restore default models:", error)
      showToast({ message: t("models.modelSaveFailed"), type: "error" })
    }
  }
                ...
                <div className="providers-list-item" key={`multiModelConfig-${index}`}>
                  ...
                  <div className="api-key">
                    <div>
                      ...
                      {multiModelConfig.accessKeyId && <div>KeyId： ***{multiModelConfig.accessKeyId.slice(-5)}</div>}
                      {multiModelConfig.secretAccessKey && <div>SecretKey： ***{multiModelConfig.secretAccessKey.slice(-5)}</div>}
